...
int main()
{
    ...
    else if (strcmp(method, "POST") == 0)
    {
        ...
        if (action)
        {
            ...
            else if (strncmp(action, "ByMake", 6) == 0)
            {
                printf("Content-type: text/html\n\n");

                //get the needed (2) pieces of information needed for the query
                char *car_data = strstr(post_data_2, "ByMake");
                if (car_data)
                {
                    car_data += 6;
                    ...
                    sqlite3* db = Connect();
                    sqlite3_stmt *stmt;
                    char query[] = "SELECT * FROM Car WHERE Make = ?";
                    int rc = sqlite3_prepare_v2(db, query, -1, &stmt, NULL);
                    //bind the value
                    sqlite3_bind_int(stmt, 1, make_id);
            ...
            else if (strncmp(action, "ByLicPlate", 10) == 0)
            {
                printf("Content-type: text/html\n\n");

                //get the needed (2) pieces of information needed for the query
                char *car_data = strstr(post_data_2, "ByLicPlate");
                if (car_data)
                {
                    car_data += 11;

                    char *plate = strstr(post_data_3, "licplate=") + 9;

                    //set up the proper query
                    sqlite3* db = Connect();
                    sqlite3_stmt *stmt;
                    char query[] = "SELECT * FROM Car WHERE LicPlate = ?";
                    int rc = sqlite3_prepare_v2(db, query, -1, &stmt, NULL);
                    //bind the value
